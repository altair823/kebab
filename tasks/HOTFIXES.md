...
`OnnxPaddleOcr::run_rec` 은 검출된 박스를 높이 48 로 리사이즈하면서 **폭을 1 이상으로만** 보장했다. 그런데 PP-OCRv5 rec 백본은 폭을 반복해서 줄이기 때문에 **폭 4 이하** 입력은 도중에 폭 0 인 특징맵이 되고, ORT 가 Conv 에서 세션 실행 전체를 실패시킨다. (이슈 본문과 이 항목의 초안이 "한 자릿수" 라고 적었으나 아래 표대로 폭 5~9 는 정상 동작한다.)
...
`Clickpath_Analysis.png` 이 잘 보여 준다. 이 이미지는 박스를 **119 개** 검출하는데, 수정 전에는 그중 **3 개**를 인식한 시점에 얇은 조각을 만나 세션이 죽고 그 3 개까지 함께 버려져 본문이 0 자였다. 수정 후에는 87 개 영역 1,116 자가 나온다. (초안이 "이미 인식된 87 개 영역을 잃었다" 고 적었는데 이는 틀렸다 — 87 은 수정 **후** 나오는 영역 수이고, 중단 시점에 인식돼 있던 것은 3 개다. 계측으로 확인: `ABORT regions_already_recognized=3 of detected_boxes=119`.)
