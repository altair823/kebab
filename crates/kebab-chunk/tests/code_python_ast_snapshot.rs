...
fn code_python_ast_chunks_snapshot() {
    ...
    let chunks = CodeAstV1Chunker::for_lang("python").chunk(&doc, &policy).expect("chunk");
...
fn code_python_ast_chunks_are_deterministic() {
    let policy = fixed_policy();
    let baseline: Vec<String> = CodeAstV1Chunker::for_lang("python")
        ...
        .collect();
    for _ in 0..5 {
        let again: Vec<String> = CodeAstV1Chunker::for_lang("python")
            .chunk(&fixed_doc(), &policy)
            .unwrap()
            .into_iter()
            .map(|c| c.chunk_id.0)
