//! Ingest pipeline — the orchestrator (`ingest_with_config`) plus its
//! per-asset media handlers (markdown / image / pdf / code), stage helpers
//! (fingerprint-and-skip, document-record store, chunking, embed-with-cache),
//! the deleted-file sweep, and the ingest-config signature that drives the
//! version cascade.
//!
//! Split out of the `kebab-app` `lib.rs` monolith (spine Phase 3) as a pure
//! code move — no logic change. The public entry points
//! ([`ingest`], [`ingest_with_config`], [`ingest_file_with_config`],
//! [`ingest_stdin_with_config`]) plus [`IngestOpts`] are re-exported at the
//! crate root from `lib.rs`, so `kebab_app::ingest*` paths are unchanged.
//!
//! Config seam (`*_with_config`): see the crate-level docs in `lib.rs`.
...
use kebab_core::{
    Block, CanonicalDocument, Chunk, ChunkPolicy, Chunker, ChunkerVersion, DocFilter,
    DocumentStore, Embedder, EmbeddingInput, EmbeddingKind, ExtractContext, IngestReport, Lang,
    LanguageModel, MediaType, ParserVersion, RawAsset, SourceScope, SourceType, SourceUri,
    TrustLevel, VectorRecord, VectorStore,
};
...
use crate::app::App;
use crate::{NO_EXT_SENTINEL, load_config};
